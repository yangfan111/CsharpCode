...
public class MB3_TextureBaker : MB3_MeshBakerRoot
{
    ...
    */
    ...
    /// 创建图集的协程
    /// </summary>
    /// <param name="progressInfo">进度更新处理函数</param>
    /// <param name="coroutineResult">记录创建图集的写成的结果</param>
    /// <param name="saveAtlasesAsAssets">是否保存图集资源</param>
    /// <param name="editorMethods">编辑器方法接口</param>
    /// <param name="maxTimePerFrame">每帧最大的时间数</param>
    /// <returns></returns>
    public IEnumerator CreateAtlasesCoroutine(ProgressUpdateDelegate progressInfo, CreateAtlasesCoroutineResult coroutineResult, bool saveAtlasesAsAssets = false, MB2_EditorMethodsInterface editorMethods = null, float maxTimePerFrame = .01f)
    {
        ...
        //Do the material combining. 执行材质合并
        for (int i = 0; i < OnCombinedTexturesCoroutineAtlasesAndRects.Length; i++)
        {
            Material resMatToPass = null;                   // 记录合并后的最终材质
            List<Material> sourceMats = null;               // 记录待合并材质列表
            if (_doMultiMaterial)
            {
    ...
    bool _ValidateResultMaterials()
    {
        ...
        for (int i = 0; i < resultMaterials.Length; i++)
        {
            ...
            if (mm.combinedMaterial == null)                                // 合并后的材质为null
            {
                Debug.LogError("Combined Material is null please create and assign a result material.");
                return false;
            }
            ...
            {
                if (mm.sourceMaterials[j] == null)                          // 原材质为null
                {
                    Debug.LogError("There are null entries in the list of Source Materials");
                    return false;
                }

                if (targShader != mm.sourceMaterials[j].shader)             // 原材质中存在不同shader的材质球
                {
                ...
                if (allMatsInMapping.Contains(mm.sourceMaterials[j]))       // 原材质在列表中重复出现了多次
                {
        ...
        if (allMatsOnObjs.IsProperSubsetOf(allMatsInMapping))               // 真子集判断，存在源网格列表中未使用的材质球
        {
