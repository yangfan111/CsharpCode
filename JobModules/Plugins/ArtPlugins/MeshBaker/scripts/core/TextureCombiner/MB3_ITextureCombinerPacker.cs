    ...
    internal abstract class MB3_TextureCombinerPackerRoot : MB_ITextureCombinerPacker
    {
        ...
        internal static void CreateTemporaryTexturesForAtlas(List<MB_TexSet> distinctMaterialTextures, MB3_TextureCombiner combiner, int propIdx, MB3_TextureCombinerPipeline.TexturePipelineData data)
        {
            for (int texSetIdx = 0; texSetIdx < data.distinctMaterialTextures.Count; texSetIdx++)                           // 对于待合并的每一个材质
            {
                ...
                MeshBakerMaterialTexture matTex = txs.ts[propIdx];                                                          // 获取材质中指定的纹理属性
                if (matTex.isNull)                                                                                          // 该属性不存在
                {
                    ...
                    txs.CreateColoredTexToReplaceNull(data.texPropertyNames[propIdx].name, propIdx, data._fixOutOfBoundsUVs, combiner, col);        // 创建一个color tex代替不存在的纹理属性
                }
            }
        }
