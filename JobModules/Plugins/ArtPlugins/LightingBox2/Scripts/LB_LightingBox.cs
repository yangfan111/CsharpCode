...
public class LB_LightingBox : EditorWindow
{
    ...
    private bool isExpandRecords = false;
    private Vector2 recordPos = Vector2.zero;
    private SceneAsset sceneAsset = null;
    private LB_LightingProfile sceneProfile = null;

     void NewSceneInit()
    {
        if (helper == null)
        {
            GameObject helperGo = GameObject.Find("LightingBox_Helper");
            if (helperGo != null) helper = helperGo.GetComponent<LB_LightingBoxHelper>();
        }
        ...
        {
            // string path = EditorPrefs.GetString(sceneName);
            ...
            // else
            // {
            //     LB_LightingProfile = AssetDatabase.LoadAssetAtPath<LB_LightingProfile>(path);
            //     helper.Update_MainProfile(LB_LightingProfile);
            // }

            LB_LightingProfile = helper.mainLightingProfile; // LightingBoxScenesRecord.instance.GetRecord(scene.path);
            if (LB_LightingProfile == null)
            {
                LB_LightingProfile = Resources.Load("DefaultSettings") as LB_LightingProfile;
            }
            helper.Update_MainProfile(LB_LightingProfile);
    ...
    void OnNewSceneOpened(Scene scene, OpenSceneMode mode)
    {
        Scene sc = EditorSceneManager.GetActiveScene();
        if (sc.name.Equals(currentScene)) return;
    ...
    void OnDisable()
    {
        ...
        EditorSceneManager.sceneOpened -= OnNewSceneOpened;
    ...
    void OnGUI()
    {
        ...
        if (winMode == WindowMode.Part1)
        {
            ...
            if (profileState)
            {
                ...
                if (lightingProfileRef != LB_LightingProfile)
                {
                    ...
                    LightingBoxScenesRecord.instance.UpdateRecord(scenePath, LB_LightingProfile);
        ...
        #region records
        EditorGUILayout.Space();
        isExpandRecords = EditorGUILayout.Foldout(isExpandRecords, "Records");
        if (isExpandRecords)
        {
            EditorGUI.indentLevel++;
            recordPos = EditorGUILayout.BeginScrollView(recordPos, GUI.skin.box, GUILayout.MinHeight(150f));
            EditorGUI.BeginDisabledGroup(true);
            var records = LightingBoxScenesRecord.instance.records;
            foreach (var record in records)
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.ObjectField(record.scene, typeof(SceneAsset), false);
                EditorGUILayout.ObjectField(record.profile, typeof(LB_LightingProfile), false);
                EditorGUILayout.EndHorizontal();
            }
            ...
            sceneProfile = EditorGUILayout.ObjectField(sceneProfile, typeof(LB_LightingProfile), false) as LB_LightingProfile;
            if (GUILayout.Button("Add", GUILayout.MaxWidth(50f)))
            {
                if (sceneAsset == null || sceneProfile == null)
                {
                    Debug.LogError("lighting box add error, sceneProfile or sceneAsset is null");
                }
                else
                {
                    string path = AssetDatabase.GetAssetPath(sceneAsset);
                    LightingBoxScenesRecord.instance.TryUpdateOrAddRecord(path, sceneProfile);
                    sceneAsset = null;
                    sceneProfile = null;
                }
            }
            ...
            EditorGUI.indentLevel--;
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("Save Current Scene Record"))
            {
                Scene scene = EditorSceneManager.GetActiveScene();
                LightingBoxScenesRecord.instance.TryUpdateOrAddRecord(scene.path, LB_LightingProfile);
            }
            if (GUILayout.Button("Delete Current Scene Record"))
            {
                Scene scene = EditorSceneManager.GetActiveScene();
                LightingBoxScenesRecord.instance.RemoveRecord(scene.path);
            }
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.Space();
        }
    ...
    void OnLoad()
    {
        ...
        if (!mainCamera)
        {
            if (LB_LightingProfile == null) return;
