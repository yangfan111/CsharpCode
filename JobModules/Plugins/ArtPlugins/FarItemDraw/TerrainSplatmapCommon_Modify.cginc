...
struct Input
{
    ...
    UNITY_FOG_COORDS(5)
#if FAR_FOD_ON
		half dis : TEXCOORD6;  // Not prefixing '_Contorl' with 'uv' allows a tighter packing of interpolators, which is necessary to support directional lightmap.
#endif 
};
...
void SplatmapVert(inout appdata_full v, out Input data)
{
 ...
 float4 pos = mul(UNITY_MATRIX_VP, wPos);
    UNITY_TRANSFER_FOG(data, pos);
#if FAR_FOD_ON	
	data.dis =   distance(_WorldSpaceCameraPos, wPos);
#endif
#ifdef _TERRAIN_NORMAL_MAP
