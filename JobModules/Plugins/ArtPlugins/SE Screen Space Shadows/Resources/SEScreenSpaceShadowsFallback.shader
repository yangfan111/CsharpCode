...
Shader "Hidden/SEScreenSpaceShadowsFallback"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass // 0 blend
		{
			...
			sampler2D SEScreenSpaceShadowsTexture;

			fixed4 frag (v2f input) : SV_Target
			{
				fixed4 col = tex2D(SEScreenSpaceShadowsTexture, input.uv).xxxx;
		...
		Pass // 1 render screen space shadows
		{
			...
			{
				float depth = fixDepth(tex2Dlod(_MainTex, float4(coord.xy, 0.0, 0.0)).x);
			...
			fixed4 frag (v2f input) : SV_Target
			{
				...
				for (int i = 0; i < numSamples; i++)
				{
					...
					float sampleDepth = LinearEyeDepth(tex2Dlod(_MainTex, float4(rayProjPos.xy, 0.0, 0.0)).x);
		...
		Pass // 3 bilateral blur
		{
			...
			float4 frag(v2f input) : COLOR0
			{
				...
				float depth = LinearEyeDepth(tex2Dlod(_CameraDepthTexture, float4(input.uv.xy, 0.0, 0.0)).x);
				float thresh = 0.16 * BlurDepthTolerance;


				for (int i = -1; i <= 1; i++)
				{
					float2 offs = SESSSBlurKernel.xy * (i)* ScreenRes.zw * 1.0;
					float sampleDepth = LinearEyeDepth(tex2Dlod(_CameraDepthTexture, float4(input.uv.xy + offs.xy, 0.0, 0.0)).x);
