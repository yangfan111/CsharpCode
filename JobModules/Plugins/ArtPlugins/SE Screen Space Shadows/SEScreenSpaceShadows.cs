...
//        camera.RemoveCommandBuffer(cameraEvent, commandBuffer);
	...
	void Init()
	{
	    ...
	    blendShadowsCommandBuffer = new CommandBuffer {name = "SE Screen Space Shadows: Blend"};
	    blendShadowsCommandBuffer.Blit(BuiltinRenderTextureType.None, BuiltinRenderTextureType.CurrentActive, material, 0);


	    renderShadowsCommandBuffer = new CommandBuffer {name = "SE Screen Space Shadows: Render"};

	    int resolution = highResolution ? -1 : -2;

	    int ssBuf0 = Shader.PropertyToID("SEScreenShadowBuffer0");
		int ssBuf1 = Shader.PropertyToID("SEScreenShadowBuffer1");
		int depthSource = Shader.PropertyToID("DepthSource");
		renderShadowsCommandBuffer.GetTemporaryRT(ssBuf0, resolution, resolution, 0, FilterMode.Bilinear, RenderTextureFormat.R8);
		if (bilateralBlur)
		{
			renderShadowsCommandBuffer.GetTemporaryRT(ssBuf1, resolution, resolution, 0, FilterMode.Bilinear, RenderTextureFormat.R8);
		}
		renderShadowsCommandBuffer.GetTemporaryRT(depthSource, resolution, resolution, 0, FilterMode.Point, RenderTextureFormat.RFloat);
		...
		if (bilateralBlur)
		{
			...
			{
				renderShadowsCommandBuffer.SetGlobalVector("SESSSBlurKernel", new Vector2(0.0f, 1.0f));
				renderShadowsCommandBuffer.Blit(ssBuf0, ssBuf1, material, 3);
				renderShadowsCommandBuffer.SetGlobalVector("SESSSBlurKernel", new Vector2(1.0f, 0.0f));
				renderShadowsCommandBuffer.Blit(ssBuf1, ssBuf0, material, 3);
			}
		}

		renderShadowsCommandBuffer.SetGlobalTexture("SEScreenSpaceShadowsTexture", ssBuf0);
