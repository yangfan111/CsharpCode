...
namespace App.Client.ClientSystems
{
    public static class DecalMesh
    {
        ...
        // MeshCollider
        private static LayerMask careLayer;
        private const string PLAYER = "Player";
        private const string VEHICLE = "Vehicle";
        private const string IGNOREDECAL = "IgnoreDecal";
        private const string HITBOX = "Hitbox";
        private static Vector3 cacheGridSize = Vector3.zero;
        public static void Initialize()
        {
            if (initialized) return;
            ignoreLayer.value = (1 << LayerMask.NameToLayer(PLAYER) |
                1 << LayerMask.NameToLayer(VEHICLE) |
                1 << LayerMask.NameToLayer(IGNOREDECAL));
        ...
        public static Mesh Create(DecalVolume decal, float surfaceMaxAngle, float surfaceDistance, float smoothAngle, Vector3 normal, float ratio)
        {
            ...
            {
                if ((1 << terrains[i].gameObject.layer & ignoreLayer.value) > 0) continue;
        ...
        private static Vector3[] PushMesh(Mesh mesh, float distance, Vector3 normal, float ratio)
        {
            ...
            {
                vertices[i] += (normals[i] * distance + normal * ratio);
            }
        ...
        private static List<Polygon> TerrainPlane(Vector3 gridStart, float gridSize, Vector3 start, Vector3 end, Vector3 planeSize)
        {
            ...
            for (int x = 0; x < sizeX; x++)
            {
                ...
                {
                    cacheGridSize.x = x * gridSize;
                    cacheGridSize.z = z * gridSize;
