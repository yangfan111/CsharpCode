...
{
    public class PointerProvider : AbstractProvider<IPointerReceiver, PointerData>
    {
        ...
        protected override List<UserInputKey> GetKeyList(IPointerReceiver data)
        {
            _resultList.Clear();
            var binding = data.GetBinding();
            foreach (var keyValuePair in binding)
            {
                _resultList.Add(keyValuePair.Key.Key);
            }
        ...
        protected override void DoCollect()
        {
            foreach (var data in _collection)
            {
                ...
                {
                    var keys = _keyConverter.Convert(inputKey);
