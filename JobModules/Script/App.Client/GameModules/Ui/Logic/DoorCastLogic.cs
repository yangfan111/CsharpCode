    ...
    public class DoorCastLogic : AbstractCastLogic
    {
        ...
        public override void OnAction()
        {
            if (_enableAction)
            {
                ...
                _userCmdGenerator.SetUserCmd((cmd) => cmd.UseEntityId = _doorEntityId);
        ...
        {
            _doorEntityId = DoorCastData.EntityId(data.IdList);
            var door = _doorContext.GetEntityWithEntityKey(new Core.EntityComponent.EntityKey(_doorEntityId, (short) EEntityType.MapObject));
            _enableAction = false;
            
            if (door.hasDoorData && door.hasRawGameObject)
            {
                var player = _playerContext.flagSelfEntity;
