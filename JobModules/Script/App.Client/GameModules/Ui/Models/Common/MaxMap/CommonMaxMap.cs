...
namespace App.Client.GameModules.Ui.Models.Common
{
    public partial class CommonMaxMap : ClientAbstractModel, IUiHfrSystem
    {
        ...
        private Transform c4Locate = null;
...
//           MaxMapRepresentWHByRice = adapter.MapShowSizeByRice.x;
        ...
        private void UpdateC4()
        {
            ...
            if (adapter.IsC4Drop)
            {
                var pos = (new Vector2(adapter.C4DropPosition.x, adapter.C4DropPosition.z) - PlayRefePosByRice) * rate;
                c4Locate.GetComponent<RectTransform>().anchoredPosition = playRefePosByPixel + pos;                  //更新标记位置
            }
        }
...
//            canvasScaleCom.referenceResolution = new Vector2(Screen.width, Screen.height);
            ...
            root.GetComponent<RectTransform>().sizeDelta = new Vector2(maskWh, maskWh);
            root.GetComponent<RectTransform>().anchoredPosition = Vector2.zero;
            maxMapMaskControlRect.sizeDelta = new Vector2(maskWh, maskWh);

            maxMapMaskWByPixel = maxMapMaskControlRect.sizeDelta.x;
            maxMapMaskHByPixel = maxMapMaskControlRect.sizeDelta.y;
            rate = maxMapMaskWByPixel / MaxMapRepresentWHByRice;
            lastRate = rate;
