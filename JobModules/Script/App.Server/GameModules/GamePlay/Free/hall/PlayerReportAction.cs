...
namespace App.Server.GameModules.GamePlay.Free.hall
{
    ...
    class PlayerReportAction : AbstractPlayerAction
    {
                ...
                if (issend)
                {
                    switch (rankType)
                    {
                        ...
                        case RankType.Survival:
                            ...
                            if (modeId != GameRules.SoloSurvival && modeId != GameRules.LadderSoloSurvival)
                            {
                                SurvivalRank(playerInfoList.Values.ToList().Union(leavedInfoList.Values.ToList()).ToList(), goPlayerList, modeId);
                            }
                            break;
                        default:
                            break;
        ...
        private void SurvivalRank(List<IPlayerInfo> players, Dictionary<long, GameOverPlayer> goPlayerList, int modeId)
        {
            if (players.Count < 1)
                return;
