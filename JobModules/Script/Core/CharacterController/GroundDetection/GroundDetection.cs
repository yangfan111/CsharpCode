...
//        private bool CandidateGroundDetecte(Vector3 position, Quaternion rotation, float distance,
//            out RaycastHit hitInfo, ControllerHitInfo controllerHitInfo)
//        {
//            var ret = false;
//            ret = BottomSphereCast(position, rotation, out hitInfo, distance);
//            if (!ret && controllerHitInfo.Valid)
//            {
//                Logger.InfoFormat("first sphere cast failed!!!");
//                ret = BottomSphereCast(position, rotation, out hitInfo, controllerHitInfo.MoveLength + kControllerCastOffset, _backstepDistance);
//                if (ret)
//                {
//                    Logger.InfoFormat("{0} is not detect, but {1} is detected, back:{2}, v1:{3}", distance, hitInfo.distance, _backstepDistance,hitInfo.collider.name);
//                }
//                else
//                {
//                    ret = BottomSphereCast(position, rotation, out hitInfo, 10f);
//                    if (ret)
//                    {
//                        Logger.InfoFormat("{0} is not detect, but {1} is detected, back:{2},v2:{3}", distance, hitInfo.distance, _backstepDistance, hitInfo.collider.name);
//                    }
//                }
//            }
//
...
//            {
//                var radius = capsuleCollider.radius;
//                DebugDraw.EditorDrawCapsule(position + new Vector3(0f,radius,0f),position + new Vector3(0f,radius,0f), radius, Color.green);
//                Debug.DrawRay(hitInfo.point, hitInfo.normal * 8, Color.magenta, 1, false);
//            }
//            return ret;
//        }
