...
namespace Core.EntitasAdpater
{
    public class PartialSnapshotFactory : BaseRefCounter
    {
        ...
        public ISnapshot GeneratePartialSnapshot(int seq,
            ...
            bool isAccountStage,
            bool isPrepareStage,
            IWatchDict watchDict = null, 
            Action<IGameEntity, bool, IWatchDict> OnInsertFun = null
            )
        {
            _preEntitys.Clear();
            _seq = seq;
        ...
        private IWatchDict _watchDict = null;      // 上次残留的感知对象字典
        private Action<IGameEntity, bool, IWatchDict> _OnInsertFun = null;    //插入快照时的响应函数
        ...
        public void DoInsert(IGameEntity localEntity)
        {
            ...
            {
                _OnInsertFun(localEntity, forPre, _watchDict);
            }
        }
    ...
    public class SnapshotFactory
    {
        ...
        public ISnapshot GeneratePerPlayerSnapshot(int seq, EntityKey self, Vector3 position, Bin2DConfig config,
            IBin2DManager bin, List<IGameEntity> preEntitys, bool isAccountStage, bool isPrepareStage,
            IWatchDict watchDict, Action<IGameEntity, bool, IWatchDict> OnInsertFun)
        {
            var factory = ObjectAllocatorHolder<PartialSnapshotFactory>.Allocate();
            var ret = factory.GeneratePartialSnapshot(seq, self, position, config, bin, preEntitys, isAccountStage, isPrepareStage, watchDict, OnInsertFun);
            factory.ReleaseReference();
            return ret;
        }
