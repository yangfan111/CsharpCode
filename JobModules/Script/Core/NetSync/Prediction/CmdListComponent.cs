...
namespace App.Shared.Components.Player
{
    ...
    public class CmdListComponent<TCmd> :IResetableComponent where TCmd : class, ICmd, IRefCounter
    {
        ...
        public List<TCmd> GetLargerThan(int lastSeq)
        {
            ...
            Core.Utils.LinkedListNode<TCmd> head=null;
            var last = _userCmdList.Last;
            if (last != null && last.Value.Seq > lastSeq)
            {
                head = _userCmdList.First;
                while (head != null)
                {
                    var cmd = head.Value;
            ...
            if (head == null) head = _userCmdList.Last;
            while (head != null)
            {
                var cmd = head.Value;
                head = head.Previous;
