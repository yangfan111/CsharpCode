...
public static class DebugDraw
{
	...
	public static void DebugPoint(Vector3 position, Color color, float scale = 1.0f, float duration = 0, bool depthTest = true)
	{
	    ...
	    RuntimeDebugDraw.Draw.DrawRay(position+(Vector3.up*(scale*0.5f)), -Vector3.up*scale, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawRay(position+(Vector3.right*(scale*0.5f)), -Vector3.right*scale, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawRay(position+(Vector3.forward*(scale*0.5f)), -Vector3.forward*scale, color, duration, depthTest);
	}
	...
	public static void DebugBounds(Bounds bounds, Color color, float duration = 0, bool depthTest = true)
	{
		...
		RuntimeDebugDraw.Draw.DrawLine(ruf, luf, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(ruf, rub, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(luf, lub, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(rub, lub, color, duration, depthTest);
		
		RuntimeDebugDraw.Draw.DrawLine(ruf, rdf, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(rub, rdb, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(luf, lfd, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(lub, lbd, color, duration, depthTest);
		
		RuntimeDebugDraw.Draw.DrawLine(rdf, lfd, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(rdf, rdb, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(lfd, lbd, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(lbd, rdb, color, duration, depthTest);
	}
	...
	public static void DebugLocalCube(Transform transform, Vector3 size, Color color, Vector3 center = default(Vector3), float duration = 0, bool depthTest = true)
	{
		...
		RuntimeDebugDraw.Draw.DrawLine(lbb, rbb, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(rbb, lbf, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(lbf, rbf, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(rbf, lbb, color, duration, depthTest);
		
		RuntimeDebugDraw.Draw.DrawLine(lub, rub, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(rub, luf, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(luf, ruf, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(ruf, lub, color, duration, depthTest);
		
		RuntimeDebugDraw.Draw.DrawLine(lbb, lub, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(rbb, rub, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(lbf, luf, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(rbf, ruf, color, duration, depthTest);
	}
	...
	public static void DebugCircle(Vector3 position, Vector3 up, Color color, float radius = 1.0f, float duration = 0, bool depthTest = true)
	{
		...
		for(var i = 0; i < 91; i++){
			...
			RuntimeDebugDraw.Draw.DrawLine(_lastPoint, _nextPoint, color, duration, depthTest);
	...
	public static void DebugWireSphere(Vector3 position, Color color, float radius = 1.0f, float duration = 0, bool depthTest = true)
	{
		...
		for(int i = 1; i < 37; i++){
            ...
            RuntimeDebugDraw.Draw.DrawLine(x, new_x, color, duration, depthTest);
			RuntimeDebugDraw.Draw.DrawLine(y, new_y, color, duration, depthTest);
			RuntimeDebugDraw.Draw.DrawLine(z, new_z, color, duration, depthTest);
	...
	public static void DebugCylinder(Vector3 start, Vector3 end, Color color, float radius = 1, float duration = 0, bool depthTest = true)
	{
		...
		//Side lines
		RuntimeDebugDraw.Draw.DrawLine(start+right, end+right, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(start-right, end-right, color, duration, depthTest);
		
		RuntimeDebugDraw.Draw.DrawLine(start+forward, end+forward, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(start-forward, end-forward, color, duration, depthTest);
		
		//Start endcap
		RuntimeDebugDraw.Draw.DrawLine(start-right, start+right, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(start-forward, start+forward, color, duration, depthTest);
		
		//End endcap
		RuntimeDebugDraw.Draw.DrawLine(end-right, end+right, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawLine(end-forward, end+forward, color, duration, depthTest);
	}
	...
	public static void DebugCone(Vector3 position, Vector3 direction, Color color, float angle = 45, float duration = 0, bool depthTest = true)
	{
		...
		RuntimeDebugDraw.Draw.DrawRay(position, slerpedVector.normalized*dist, color);
		RuntimeDebugDraw.Draw.DrawRay(position, Vector3.Slerp(_forward, -_up, angle/90.0f).normalized*dist, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawRay(position, Vector3.Slerp(_forward, _right, angle/90.0f).normalized*dist, color, duration, depthTest);
		RuntimeDebugDraw.Draw.DrawRay(position, Vector3.Slerp(_forward, -_right, angle/90.0f).normalized*dist, color, duration, depthTest);
	...
	{
		RuntimeDebugDraw.Draw.DrawRay(position, direction, color, duration, depthTest);
	...
	public static void DebugCapsule(Vector3 start, Vector3 end, Color color, float radius = 1, float duration = 0, bool depthTest = true)
	{
		...
		for(int i = 1; i < 26; i++){
			...
			//Start endcap
			RuntimeDebugDraw.Draw.DrawLine(Vector3.Slerp(right, -up, i/25.0f)+start, Vector3.Slerp(right, -up, (i-1)/25.0f)+start, color, duration, depthTest);
			RuntimeDebugDraw.Draw.DrawLine(Vector3.Slerp(-right, -up, i/25.0f)+start, Vector3.Slerp(-right, -up, (i-1)/25.0f)+start, color, duration, depthTest);
			RuntimeDebugDraw.Draw.DrawLine(Vector3.Slerp(forward, -up, i/25.0f)+start, Vector3.Slerp(forward, -up, (i-1)/25.0f)+start, color, duration, depthTest);
			RuntimeDebugDraw.Draw.DrawLine(Vector3.Slerp(-forward, -up, i/25.0f)+start, Vector3.Slerp(-forward, -up, (i-1)/25.0f)+start, color, duration, depthTest);
			
			//End endcap
			RuntimeDebugDraw.Draw.DrawLine(Vector3.Slerp(right, up, i/25.0f)+end, Vector3.Slerp(right, up, (i-1)/25.0f)+end, color, duration, depthTest);
			RuntimeDebugDraw.Draw.DrawLine(Vector3.Slerp(-right, up, i/25.0f)+end, Vector3.Slerp(-right, up, (i-1)/25.0f)+end, color, duration, depthTest);
			RuntimeDebugDraw.Draw.DrawLine(Vector3.Slerp(forward, up, i/25.0f)+end, Vector3.Slerp(forward, up, (i-1)/25.0f)+end, color, duration, depthTest);
			RuntimeDebugDraw.Draw.DrawLine(Vector3.Slerp(-forward, up, i/25.0f)+end, Vector3.Slerp(-forward, up, (i-1)/25.0f)+end, color, duration, depthTest);
		}
	}
	...
	public static void EditorDrawCone(Vector3 position, Vector3 direction, Color color, float angle = 45)
	{
		float length = direction.magnitude;
		...
		Debug.DrawRay(position, slerpedVector.normalized*dist);
		Debug.DrawRay(position, Vector3.Slerp(_forward, -_up, angle/90.0f).normalized*dist);
		Debug.DrawRay(position, Vector3.Slerp(_forward, _right, angle/90.0f).normalized*dist);
		Debug.DrawRay(position, Vector3.Slerp(_forward, -_right, angle/90.0f).normalized*dist);
		
		DebugDraw.EditorDrawCircle(position+_forward, direction, color, (_forward-(slerpedVector.normalized*dist)).magnitude);
		DebugDraw.EditorDrawCircle(position+(_forward*0.5f), direction, color, ((_forward*0.5f)-(slerpedVector.normalized*(dist*0.5f))).magnitude);
	}
	...
	public static void EditorDrawArrow(Vector3 position, Vector3 direction, Color color, float duration = 0.0f, bool depthTest = false)
	{	
		Debug.DrawRay(position,direction,color, duration, depthTest);
		EditorDrawCone(position + direction, -direction * 0.333f, color, 15);
	}
	...
	{
		DrawCapsule(start, end, Color.red, radius);
	}

	public static void EditorDrawCapsule(Vector3 start, Vector3 end, float radius, Color color, float duration = 0f, bool depthTest = true)
	{
		Vector3 up = (end-start).normalized*radius;
		Vector3 forward = Vector3.Slerp(up, -up, 0.5f);
		Vector3 right = Vector3.Cross(up, forward).normalized*radius;
		...
		//Radial circles
		EditorDrawCircle(start, up, color, radius, duration, depthTest);	
		EditorDrawCircle(end, -up, color, radius, duration, depthTest);
	...
	public static void EditorDrawCircle(Vector3 position, Vector3 up, Color color, float radius, float duration = 0f, bool depthTest = true)
	{
		up = ((up == Vector3.zero) ? Vector3.up : up).normalized * radius;
		Vector3 _forward = Vector3.Slerp(up, -up, 0.5f);
		Vector3 _right = Vector3.Cross(up, _forward).normalized*radius;
