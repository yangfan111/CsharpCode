...
namespace App.Shared.GameModules.Weapon
{
    public class BulletPlayerUtil
    {
        ...
        {
            DoProcessPlayerHealthDamage(contexts, damager, srcPlayer, playerEntity, damage);
        }

        public static void DoProcessPlayerHealthDamage(Contexts contexts, IPlayerDamager damager, PlayerEntity srcPlayer, PlayerEntity playerEntity, PlayerDamageInfo damage)
        {
        ...
        private static List<PlayerEntity> OnePlayerHealthDamage(Contexts contexts, IGameRule gameRule, PlayerEntity srcPlayer, PlayerEntity playerEntity, PlayerDamageInfo damage, bool isTeam)
        {
            ...
            if (srcPlayer != null)
            {
                ...
                if (SharedConfig.IsServer)
                {
                    if (damage.type == (int) EUIDeadType.Weapon || damage.type == (int) EUIDeadType.Unarmed)
                    {
                        ...
                        if (newConfig != null && newConfig.SubType != (int) EWeaponSubType.Grenade)
                        {
            ...
            damage.damage = playerEntity.gamePlay.DecreaseHp(realDamage);
                        ...
                        if (damage.HitPoint != Vector3.zero && damage.HitDirection != Vector3.zero && playerEntity.hasRagDoll)
                        {
                            ...
                            switch (damage.type)
                            {
                                case (int) EUIDeadType.Weapon:
                                    ...
                                    if (config.NewWeaponCfg.Type == (int) EWeaponType_Config.ThrowWeapon)
                                    {
                                    ...
                                    else
                                    {
                                        foreach (var pair in Joint2BodyPart)
                                        {
                                            ...
                                            {
                                                playerEntity.ragDoll.RigidBodyTransformName =
                                                    pair.Key.Substring(0, pair.Key.LastIndexOf("_"));
                                                break;
                                            }
                                        }
        ...
        private static List<PlayerEntity> CheckUpdatePlayerStatus(PlayerEntity player, PlayerDamageInfo damage, Contexts contexts)
        {
            ...
            if (gamePlay.IsLifeState(EPlayerLifeState.Alive))
            {
                if (gamePlay.CurHp <= 0)
                {
                    if (damage.type == (int) EUIDeadType.Bombing)
                    {
                        gamePlay.ChangeLifeState(EPlayerLifeState.Dead, player.time.ClientTime);
                        return null;
                    }
                    ...
                    else if (contexts.session.commonSession.RoomInfo.TeamCapacity > 1 && aliveTeams.Count >= 1)
                    {
                        //受伤状态
        ...
        public static void ProcessDamageStatistics(Contexts contexts, IGameRule gameRule, PlayerEntity srcPlayer, PlayerEntity targetPlayer, PlayerDamageInfo damage)
        {
            ...
            if (null != srcPlayer)
            {
                ...
                targetPlayer.statisticsData.AddOtherInfo(srcPlayer.entityKey.Value, damage.weaponId, isKill, isHitDown, damage.damage, srcPlayer.playerInfo, System.DateTime.Now.Ticks / 10000);
                ...
                if (isTargetDead)
                {
                    if (!isTeammate)
                    {
                        ...
                        srcPlayer.statisticsData.Statistics.LastKillTime = DateTime.Now.Ticks / 10000L;
                        //爆头击杀（不包括近战）
            ...
            if (isTargetDead)
            {
                ...
                targetPlayer.statisticsData.Statistics.LastDeadTime = DateTime.Now.Ticks / 10000L;
                if (null == srcPlayer)
                {
        ...
        private static float DamageFactor(float damage)
        {
            if (damage > 0 && damage < 31)
                return 0;
            if (damage >= 31 && damage < 60)
                return 0.5f;
            if (damage >= 60 && damage < 90)
                return 0.75f;
            if (damage >= 90)
                return 1;
            return 0;
