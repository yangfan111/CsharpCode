...
namespace App.Shared.GameModules.Player
{
    public static class PlayerEntityUtility
    {
        ...
        {
            var config = SingletonManager.Get<CharacterStateConfigManager>().GetCharacterControllerCapsule(PostureInConfig.Stand);
        ...
        public static Vector3 GetCameraBulletEmitPosition(PlayerEntity playerEntity)
        {
            ...
            if (playerEntity.cameraStateNew.ViewNowMode == (int)ECameraViewMode.GunSight)
            {
                ...
                var _sightLocationP1 = playerEntity.characterBoneInterface.CharacterBone.GetLocation(Utils.CharacterState.SpecialLocation.SightsLocatorPosition, Utils.CharacterState.CharacterView.FirstPerson);
        ...
        public static Vector3 GetHandWeaponPosition(this PlayerEntity playerEntity)
        {
            ...
            if (playerEntity.appearanceInterface.Appearance.IsFirstPerson)
            {
                if (null == playerEntity.playerInfo.FirstPersonRightHand)
                {
                    ...
                    playerEntity.playerInfo.FirstPersonRightHand = BoneMount.FindChildBoneFromCache(root, BoneName.CharRightHand);
                }
            ...
            else
            {
                ...
                {
                    var root = playerEntity.RootGo();
                    playerEntity.playerInfo.ThirdPersonRightHand = BoneMount.FindChildBoneFromCache(root, BoneName.CharRightHand);
                }
