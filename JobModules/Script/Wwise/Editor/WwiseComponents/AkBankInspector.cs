...
public class AkBankInspector : AkBaseInspector
{
	...
	public override void OnChildInspectorGUI()
	{
		...
		using (new UnityEditor.EditorGUILayout.VerticalScope("box"))
		{
			...
			if (decode.boolValue)
			{
				if (decode.boolValue != oldDecodeValue && AkPluginSettingsContainer.ActivePlatformSettings.AkInitializationSettings.preparePoolSize == 0)
				{
				...
				if (oldSaveDecodedValue && saveDecoded.boolValue == false)
				{
					...
					var decodedBankPath =
						System.IO.Path.Combine(AkUtilities.GetWiseBankFolder_Full_Decode(), bank.data.Name + ".bnk");
