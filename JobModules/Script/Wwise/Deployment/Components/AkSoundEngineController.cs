...
//	}
	...
	public void Init(AkInitializer akInitializer)
	{
		...
		{
#if UNITY_EDITOR
			if (AKProcessProvider.ResetSoundEngine(UnityEngine.Application.isPlaying || UnityEditor.BuildPipeline.isBuildingPlayer))
				UnityEditor.EditorApplication.update += LateUpdate;
#else
			UnityEngine.Debug.LogError("WwiseUnity: Sound engine is already initialized.");
#endif
		...
		if (!AKProcessProvider.InitializeSoundEngine())
			return;
#if UNITY_EDITOR
        if (UnityEditor.BuildPipeline.isBuildingPlayer)
            return;
        ...
        {
            if (OnAudioPluginInitialized != null)
	...
	public void Terminate()
	{
        ...
        AKProcessProvider.TerminateSoundEngine();
	}
