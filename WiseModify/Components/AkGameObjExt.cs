...
public class AkGameObjExt : UnityEngine.MonoBehaviour
{
    ...
    private bool mInitializeSucess = false;
    private void Awake()
    {
        //#if UNITY_EDITOR
...
//#endif
        ...
        var up = GetUpward();
        UnityEngine.Debug.LogWarning(m_posData);
        if (!mInitializeSucess)
        {
            return;
            //m_posData = new AkGameObjPositionData();
        }
    ...
    private class Migration14Data
    {
        ...
        public Migration14Data()
        {
            ...
            foreach (var listener in listenerObjects)
            {
                ...
                if (listener.GetComponent<AkGameObjExt>() == null)
                {
                    var akGameObj = listener.gameObject.AddComponent<AkGameObjExt>();
                    if (akGameObj)
                    {
        ...
        public void Migrate(AkGameObjExt akGameObj)
        {
            if (listeners != null)
            {
                for (var ii = 0; ii < AK_NUM_LISTENERS; ++ii)
